...
public class BufferTest {
	...
	public void testOneSong() throws Exception {
		...
		Song song = new Song(songFile.getAbsolutePath(), 201, "Lose Myself",
				(int) songFile.length());
		...
		assertTrue("The estimated size is not the actual size.\nActual Size: "
				+ song.getBuffer().size() + "\nEstimated Size: " + size, song
				.getBuffer().size() == size);

		songFile.setReadOnly();
		FileInputStream in = new FileInputStream(songFile);

		for (int i = 0; song.hasNextBufferItem(); i++) {
			byte[] test = song.getNextBufferItem();
	...
	public void testTwoSongs() throws InterruptedException {
		...
		Song song1 = new Song(songFile1.getAbsolutePath(), 201, "Lose Myself",
				(int) songFile1.length());
		Song song2 = new Song(songFile2.getAbsolutePath(), 201, "Rift",
		...
		while (!song1.finishedBuffering() || !song2.finishedBuffering()) {
			if (song1.finishedBuffering()) {
				synchronized (song1) {
					song1.wait();
				}
			}

			if (song2.finishedBuffering()) {
				synchronized (song2) {
					song2.wait();
				}
			}
		...
		assertTrue(
				"The estimated size of song 1 is not the actual size.\nActual Size: "
						+ song1.getBuffer().size() + "\nEstimated Size: "
						+ size, song1.getBuffer().size() == size);
		assertTrue(
				"The estimated size of song 2 is not the actual size.\nActual Size: "
						+ song2.getBuffer().size() + "\nEstimated Size: "
						+ size, song2.getBuffer().size() == size);
	}
